...
class AvoPlotFigure(core.AvoPlotElementBase, wx.ScrolledWindow):
    ...
    def enable_picking(self, val):
        """
        If val == True, then enables picking for this figure (i.e. selecting 
        elements within the figure by clicking on them).
        """
        self._picking_enabled = val
    ...
    def finalise(self, parent):
        ...
        #self.canvas.mpl_connect('button_press_event', self.on_mouse_button)
        self.canvas.mpl_connect('axes_enter_event', self.on_mouse_enter_axes)
        self.canvas.mpl_connect('axes_leave_event', self.on_mouse_leave_axes)
    ...
    def on_mouse_enter_axes(self, evnt):
        
        if self._is_panned or self._is_zoomed:
            axes = [x.get_mpl_axes() for x in self.get_child_elements()]
            
            #ensure that the zoom/pan functions only get called on the uppermost axes
            for ax in axes[:axes.index(evnt.inaxes)]:
                ax.set_navigate(False)
    
    
    def on_mouse_leave_axes(self, evnt):
        
        if self._is_panned or self._is_zoomed:
            axes = [x.get_mpl_axes() for x in self.get_child_elements()]
            
            #re-enable zoom/pan tools for the lower axes
            for ax in axes[:axes.index(evnt.inaxes)]:
